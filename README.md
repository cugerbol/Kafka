...
* В первом разделе  проекта **Node preparation** происходит подготовка i-го узла кластера (Consumer и Producer настраиваются одинаково). 
* Во втором разделе **Setup node step by step** показан процесс установки Kafka на i-й узел. 
* В третьем разделе **Fast install kafka** инструкция по быстрой установки i-го узла кластера с помошью bash скрипта.  
* В четвертом разделе **Set host** инструкция настройки видимости узлов
